...
export type ConciergeMetadata = { answerType: "canonical-qa" | "place-search" | "retrieval" | "itinerary" | "booking-safe" | "live-safe" | "emergency" | "security" | "unknown" | "provider"; confidence: number; knowledgeIds: string[]; qaIds: string[]; primaryIntent?: string; sourceIds: string[]; freshness: string[]; liveDataNeeded: boolean; language: string; provider: "deterministic" | "openai" | "ollama" }
export type ConciergeAnswer = { answer: string; metadata: ConciergeMetadata; links?: { id: string; label: string; href: string }[]; map?: import("@/lib/local-places-types").ConciergeMapPayload }
