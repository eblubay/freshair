...
export const bookingConsentSchema = z.object({ termsAccepted: z.literal(true), privacyAcknowledged: z.literal(true), houseRulesAccepted: z.literal(true), termsVersion: z.literal("2026-08-11"), privacyVersion: z.literal("2026-08-11"), houseRulesVersion: z.literal("2026-08-11") })
...
export async function createQuote(input: unknown): Promise<Quote> {
	...
	return { quoteId: nanoid(), expiresAt: new Date(Date.now() + 10 * 60_000).toISOString(), nights, currency: settings.currency, nightly, subtotal, cleaningFee: settings.cleaning_fee, taxes, discount, total: taxable + taxes, directBookingEnabled: settings.direct_booking_enabled, paymentsEnabled: settings.payments_enabled, paymentProvider: settings.payment_provider === "STRIPE" ? "STRIPE" : "BRAINTREE", cancellationPolicy, bookingPolicyConfigured: Boolean(cancellationPolicy), checkinTime: String(settings.checkin_time ?? "15:00"), timezone: String(settings.timezone ?? "America/Los_Angeles"), fullRefundDays: Number(settings.full_refund_days ?? 14), partialRefundStartDays: Number(settings.partial_refund_start_days ?? 7), partialRefundPercentage: Number(settings.partial_refund_percentage ?? 50), lateCancellationRefundPercentage: Number(settings.late_cancellation_refund_percentage ?? 0), houseRulesVersion: "2026-08-11" }
}
...
export async function createHold(input: unknown, rawGuest: unknown, clientRequestId: string, rawConsent: unknown, evidence: { ipHash?: string; userAgent?: string } = {}) {
	...
	const [settings] = await queryClient`SELECT booking_hold_minutes,checkin_time,timezone FROM booking_settings WHERE property_id = ${data.propertyId}`
	...
	try {
		...
		const [scheduled] = await tx`SELECT (${data.checkIn}::date + ${String(settings?.checkin_time ?? "15:00")}::time) AT TIME ZONE ${String(settings?.timezone ?? "America/Los_Angeles")} AS scheduled_checkin_at`
		await tx`INSERT INTO reservations (id,confirmation_code,property_id,client_request_id,guest_first_name,guest_last_name,guest_email,guest_phone,check_in,check_out,adults,children,total_guests,booking_status,payment_status,currency,subtotal,cleaning_fee,taxes,discount_amount,total_amount,amount_due,price_snapshot,scheduled_checkin_at) VALUES (${reservationId},${code},${data.propertyId},${clientRequestId},${guest.firstName},${guest.lastName},${guest.email},${guest.phone ?? null},${data.checkIn}::date,${data.checkOut}::date,${data.adults},${data.children},${data.adults + data.children},'HOLD','NOT_STARTED',${quote.currency},${quote.subtotal},${quote.cleaningFee},${quote.taxes},${quote.discount},${quote.total},${quote.total},${JSON.stringify(quote)}::jsonb,${scheduled.scheduled_checkin_at})`
		await tx`INSERT INTO booking_consents (reservation_id,terms_version,privacy_version,house_rules_version,terms_accepted,privacy_acknowledged,house_rules_accepted,house_rules_accepted_at,terms_effective_date,privacy_effective_date,evidence) VALUES (${reservationId},${consent.termsVersion},${consent.privacyVersion},${consent.houseRulesVersion},true,true,true,now(),'2026-08-11'::date,'2026-08-11'::date,${JSON.stringify(evidence)}::jsonb)`
		if (referralId) {
