...
export type Quote = { quoteId: string; expiresAt: string; nights: number; currency: string; nightly: { date: string; amount: number }[]; subtotal: number; cleaningFee: number; taxes: number; discount: number; total: number; directBookingEnabled: boolean; paymentsEnabled: boolean }
...
export async function createQuote(input: unknown): Promise<Quote> {
	...
	const [settings] = await queryClient`SELECT * FROM booking_settings WHERE property_id = ${data.propertyId}`
	...
	return { quoteId: nanoid(), expiresAt: new Date(Date.now() + 10 * 60_000).toISOString(), nights, currency: settings.currency, nightly, subtotal, cleaningFee: settings.cleaning_fee, taxes, discount, total: taxable + taxes, directBookingEnabled: settings.direct_booking_enabled, paymentsEnabled: settings.payments_enabled }
}
