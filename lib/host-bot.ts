...
export async function applyHostAction(input: unknown) {
	...
	return queryClient.begin(async (tx) => {
		...
		if (data.action === "REPLY") { await tx`UPDATE inquiries SET telegram_state='AWAITING_REPLY',telegram_draft=NULL,updated_at=now() WHERE id=${inquiryId}`; result = { state: "AWAITING_REPLY", prompt: "Write the reply. It will be previewed before sending." } }
		...
		if (data.action === "AI_DRAFT") {
			if (process.env.HOST_AI_DRAFT_ENABLED !== "true") result = { state: "UNAVAILABLE", message: "AI Draft is not currently available." }
