...
export async function captureBraintreePayment(input: unknown) {
	...
	const result = await queryClient.begin(async (tx) => {
		...
		await tx`UPDATE reservations SET booking_status='CONFIRMED', payment_status='PAID', payment_provider='BRAINTREE', provider_transaction_id=${charge.transaction.id}, amount_paid=total_amount, amount_due=0, confirmed_at=now(),california_grace_period_eligible=(scheduled_checkin_at >= now() + interval '72 hours'),california_grace_period_expires_at=CASE WHEN scheduled_checkin_at >= now() + interval '72 hours' THEN now() + interval '24 hours' ELSE NULL END, updated_at=now() WHERE id=${data.reservationId}`
