...
export function LocalGuideMap() {
	...
	return <section aria-labelledby="guide-map-heading" className="border border-[#e6ddcf] bg-white p-4 sm:p-6"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[11px] uppercase tracking-[.24em] text-[#8d7c66]">Explore nearby</p><h2 id="guide-map-heading" className="mt-2 font-serif text-3xl">The local map</h2></div><div className="flex flex-wrap gap-2">{(["All", ...GUIDE_CATEGORIES] as const).map((category) => <button key={category} type="button" onClick={() => setSelected(category)} className={`border px-3 py-2 text-[11px] uppercase tracking-[.12em] ${selected === category ? "border-[#28323b] bg-[#28323b] text-white" : "border-[#d8cdba] text-[#3d4b57]"}`}>{category}</button>)}</div></div><div ref={node} className="mt-6 h-[430px] w-full bg-[#f8f4f0]" aria-label="Interactive local guide map"/><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{visible.map((poi) => <article key={poi.name} className="border-t border-[#e6ddcf] pt-3"><p className="text-[11px] uppercase tracking-[.14em]" style={{ color: COLOURS[poi.category] }}>{poi.category} · {poi.area}</p><h3 className="mt-1 font-serif text-xl">{poi.name}</h3><p className="mt-2 text-sm leading-relaxed text-[#5d6b78]">{poi.description}</p><a className="mt-3 inline-block text-[11px] uppercase tracking-[.16em] text-[#3d4b57] underline underline-offset-4" target="_blank" rel="noreferrer" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(poi.name + " " + poi.area + " California")}`}>Directions <span aria-hidden="true">↗</span></a></article>)}</div></section>
}
