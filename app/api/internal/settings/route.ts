...
const schema = z.object({
	...
	booking: z.object({ directBookingEnabled: z.boolean(), baseNightlyRate: z.coerce.number().int().min(0), cleaningFee: z.coerce.number().int().min(0), taxRateBasisPoints: z.coerce.number().int().min(0).max(10000), minStayNights: z.coerce.number().int().min(1).max(90), maxStayNights: z.coerce.number().int().min(1).max(365), minBookingLeadHours: z.coerce.number().int().min(0).max(720), checkinTime: z.string().max(10).nullable(), checkoutTime: z.string().max(10).nullable(), holdMinutes: z.coerce.number().int().min(5).max(30), secretReleaseHours: z.coerce.number().int().min(0).max(720), fullRefundDays: z.coerce.number().int().min(1).max(90).default(14), partialRefundStartDays: z.coerce.number().int().min(1).max(89).default(7), partialRefundPercentage: z.coerce.number().int().min(0).max(100).default(50), lateCancellationRefundPercentage: z.coerce.number().int().min(0).max(100).default(0) }),
